<template>
  <div class="article">
    ...
    <b-container>
      <div class="row">
        <div id="count" class="article-id justify-content-center col-2">{{ contentId }}</div>
        <div class="article-title justify-content-center col-7">
          <h5>{{ title }}</h5>
        </div>
        <div id="name" class="article-author justify-content-end col-3">작성자 {{ user }}</div>
      </div>
      <!-- 세션값을 이용해 수정버튼을 나타내자 -->
      <div class="row">
        <div class="col-2" v-if="this.$session.get('user_idx') == this.author_idx">
          <b-button id="modifybtn" class="btn-sm" variant="outline-warning" v-on:click="editPost()">
            <span class="fas fa-edit">수정</span>
          </b-button>
        </div>
        <div class="col-2" v-else></div>
        <div class="col-7"></div>
        <div id="time" class="article-created justify-content-end col-3">
          작성일
          <small>{{ created }}</small>
        </div>
      </div>
      ...
      <div class="row text-center justify-content-center col">
        <p>
          <b-button v-on:click.passive="hitPost()">
            <span class="fas fa-thumbs-up">&nbsp;{{ hit }}</span>
          </b-button>
        </p>
      </div>
    </b-container>
