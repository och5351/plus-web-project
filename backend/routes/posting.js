...
//수정 2차 세션 확인
router.get('/Posting/sessCheckEdit?/:user_idx/:content_id', function (req, res) {
  ...
  [post_id, user_idx], function (err, row) {
    if(err){
      res.send(err)
    }else
      res.send(row)
...
router.post('/updatePost', function (req, res) {
  ...
  conn.query('UPDATE post SET board_id = ?, ca_id = ?, user_idx = ?, contents = ?, title = ?, update_date = ? WHERE post_id = ?',
